...
window.Friday = {
	...
	// ── 🐸🎮 Anuran Field Simulator — vendored froggy-bird module ──
	// The game lives in its own repo now and is vendored at
	// site/vendor/froggy-bird/ by scripts/sync-froggy.sh (same dynamic
	// injection pattern as saturday's ad-hell). This wrapper re-applies
	// the Institute's flavor on top of the module's generic defaults:
	// the birthday determination lines, the Padsworth rank, and the
	// pre-extraction localStorage keys so nobody loses a certification.
	initSimulator() {
		const root = document.getElementById('ob-sim-root');
		if (!root) return;

		const link = document.createElement('link');
		link.rel = 'stylesheet';
		link.href = 'vendor/froggy-bird/froggy-bird.css';
		document.head.appendChild(link);

		const script = document.createElement('script');
		script.src = 'vendor/froggy-bird/froggy-bird.js';
		script.onload = () => {
			if (!window.FroggyBird) return;
			window.FroggyBird.init(root, {
				storageKeys: { best: 'abri-sim-best', skin: 'abri-sim-skin' },
				environmentLines: [
					'Conditions nominal. Determination unchanged.',
					'Ambient light waning. It is still not his birthday.',
					'Passive night monitoring engaged. No birthday detected.',
					'Signal interference expected. Data integrity: nominal-ish.',
					'Cutaneous hydration: freezing. Observation continues.',
				],
				ranks: [
					{ min: 0, title: 'Trainee' },
				...
				strings: {
					canvasLabel:
						'Anuran Field Simulator — flap Specimen 7-B to clear the reeds',
				},
			});
			this._simPause = () => window.FroggyBird.pause();
		};
		script.onerror = () => {
			root.innerHTML =
				'<p class="ob-body"><em>Simulator offline. Field observation continues by other means.</em></p>';
		};
